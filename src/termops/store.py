...
class StateStore:
    """SQLite-backed store with concurrent readers and serialized writers.

    WAL journal mode allows readers to proceed without blocking writers and
    vice-versa. Each thread holds its own connection (``threading.local``), so
    reads run in parallel across threads; writes are serialized by a single
    re-entrant lock because SQLite accepts one writer at a time.
    """
    ...
    def __init__(self, path: Path):
        ...
        self._verified_tail = "0" * 64
        self._fts_enabled = False
        conn = self._connect()
        try:
            conn.execute("PRAGMA journal_mode=WAL")
            conn.executescript(SCHEMA)
            self._fts_enabled = self._probe_fts5(conn)
    ...
    def append_event(
        ...
        with self._write_lock:
            ...
            with conn:
                ...
                cursor = conn.execute(
                    "INSERT INTO events(task_id,event_type,data_json,prev_hash,event_hash,created_at) "
                    "VALUES(?,?,?,?,?,?)",
    ...
    def add_knowledge(self, task_id: str, kind: str, title: str, content: dict[str, Any]) -> dict[str, Any]:
        ...
        with self._write_lock:
            conn = self._conn()
            # Knowledge row and its FTS index must commit together so a crash
            # never leaves a row that is invisible to full-text search.
            with conn:
                cursor = conn.execute(
    ...
    def knowledge_stats(self) -> dict[str, Any]:
        ...
        conn = self._conn()
        with conn:
            total = conn.execute("SELECT COUNT(*) AS cnt FROM knowledge").fetchone()["cnt"]
    ...
    def verify_event_chain(self) -> bool:
        """Verify the append-only audit chain incrementally.

        Each event stores ``prev_hash`` and its own ``event_hash``, so the
        table is a linear hash chain. Re-hashing every row on each health
        probe would be O(n) and get slower as the log grows. Instead we hash
        only rows appended since the last verification and memoize the
        verified tail, so an unchanged chain validates in O(1) and a chain
        with ``k`` new events validates in O(k).
        """
        with self._chain_lock:
            ...
            max_seq = int(head["m"])
            if self._verified_seq == max_seq:
                return True
            previous = self._verified_tail
            rows = conn.execute(
            ...
            self._verified_seq = max_seq
            self._verified_tail = previous
            return True
    ...
    def update_task_phase(self, task_id: str, phase: MapePhase) -> Task:
        with self._write_lock:
            conn = self._conn()
            with conn:
                conn.execute("UPDATE tasks SET phase=? WHERE id=?", (phase.value, task_id))
        ...
        with self._write_lock:
            ...
            with conn:
                conn.execute(
                    ...
                    "report_json=COALESCE(?,report_json), error=?, updated_at=? WHERE id=?",
    ...
    def list_tasks(self, limit: int | None = 100, statuses: set[TaskStatus] | None = None) -> list[Task]:
        params: list[Any] = []
        ...
        if limit is not None:
            query += " LIMIT ?"
            params.append(limit)
        return [self.get_task(row["id"]) for row in self._conn().execute(query, params).fetchall()]
    ...
    def consume_login_code(self, code: str) -> bool:
        ...
        row = self._conn().execute(
            "SELECT * FROM login_codes WHERE code_hash=?", (code_hash,)
        ).fetchone()
