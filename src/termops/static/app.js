(() => {
  ...
  document.querySelectorAll('.decision-button').forEach((button) => {
    button.addEventListener('click', () => {
      ...
      document.getElementById('dialog-title').textContent = button.dataset.decision === 'approve' ? '批准命令执行' : '拒绝变更提案';
  ...
  if (taskRoot) {
    ...
    source.addEventListener('audit', (event) => {
      ...
      if (item.event_type === 'task.status') {
        ...
        if (['waiting_approval', 'succeeded', 'failed', 'cancelled'].includes(status)) {
